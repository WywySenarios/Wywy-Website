...
type ChartSettingsOption = Omit<EChartsOption, "series"> & {
  series: Array<
    Omit<LineSeriesOption, "encode"> & { encode: LineSeriesOption["encode"] }
  >;
};
...
function ChartSettings({
  ...
  setOptions: (options: ChartSettingsOption) => void;
}) {
          ...
          {columns.map((col) => (
            <div key={col} className="flex items-center gap-3">
              <Checkbox
                ...
                checked={
                  col !== x &&
                  newOptions.series.some((series) => col == series.encode.y)
                }
              ...
              <Select
                key={col + "-series-type-select"}
                defaultValue={prettySeriesTypes[0]}
                onValueChange={(val) => {
